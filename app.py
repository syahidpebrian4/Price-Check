...
def process_ocr_minimal(pil_image, master_product_names=None):
    ...
    # Menghapus elemen navigasi (Semua Kategori)
...
if menu == "📸 Image":
    ...
    st.subheader("🏷️ Price Scraper")
    with st.sidebar:
        st.subheader("🔐 LOGIN WEB")
        ig_user = st.text_input("Email/Phone:")
    ...
    if st.button("🚀 Jalankan Scraper"):
        if not urls_area or not mc_sync or not ig_user or not ig_pass:
            st.error("Lengkapi Login, URL, dan Master Code!")
        else:
            # --- KONFIGURASI KHUSUS CLOUD (Headless & System Driver) ---
            ...
            try:
                # Memanggil driver sistem yang diinstall via packages.txt
                service = Service("/usr/bin/chromedriver")
                driver = webdriver.Chrome(service=service, options=chrome_options)
                
                with st.status("Sedang bekerja...", expanded=True) as status:
                    st.write("Mencoba Login ke Indogrosir...")
                    driver.get("https://www.indogrosir.co.id/login")
                    time.sleep(3)
                    
                    try:
                        driver.find_element(By.NAME, "username").send_keys(ig_user)
                        driver.find_element(By.NAME, "password").send_keys(ig_pass)
                        driver.find_element(By.XPATH, "//button[@type='submit']").click()
                        time.sleep(5)
                        st.write("✅ Login Berhasil.")
                    except:
                        st.write("⚠️ Login otomatis gagal, mencoba akses link langsung...")
                    ...
                    for i, url in enumerate(list_urls):
                        st.write(f"Scraping {i+1}/{len(list_urls)}...")
                        driver.get(url)
                        time.sleep(5)
