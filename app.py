...
# Teks yang akan disensor otomatis (Redaksi)
TEXTS_TO_REDACT = [
...
with col_input2: date_input = st.text_input("📅 Tanggal (Ex: 23JAN2026)").strip().upper()
with col_input3: week_input = st.text_input("🗓️ Week").strip()
...
if files and m_code_input and date_input and week_input:
    try:
        ...
        st.error(f"Gagal memuat Database GSheets: {e}")
        st.stop()
    ...
    if COL_IG_NAME in db_ig.columns:
        ...
        with zipfile.ZipFile(zip_buffer, "a", zipfile.ZIP_DEFLATED, False) as zip_file:
            for idx, f in enumerate(files):
                status_text.text(f"⏳ Memproses {f.name}...")
                img_pil = Image.open(f)
                res_pcs, res_ctn, s_name, redacted_pil = process_ocr_all_prices(img_pil)
                
                # Matching Produk via Fuzzy
                ...
                if found_prodcode:
                    ...
                    if t_sheet:
                        final_list.append({
                            "prodcode": found_prodcode, "sheet": t_sheet, "index": t_idx,
                        ...
                        crop = redacted_pil.crop((0, 75, w, h)) if h > 75 else redacted_pil
                        zip_file.writestr(f"{found_prodcode}.jpg", compress_to_target(crop, TARGET_IMAGE_SIZE_KB))
        ...
        status_text.success("Selesai Memproses!")
        ...
        if final_list:
            ...
            if st.button("🚀 UPDATE HARGA KE GOOGLE SHEETS"):
